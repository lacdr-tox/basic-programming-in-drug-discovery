...
`iris[ iris$Petal.Length < 1.4,  ]` displayes all rows of the `iris` data for which the Petal Length is smaller than 1.4.  
printing the `iris$Petal.Length < 1.4` statement clearly shows what is happening.  
```{r}
iris$Petal.Length < 1.4
```
It is possible to perform multiple conditional selections by using the `&` sign inbetween.  
...
Now note that you can do calculations on booleans
```{r}
...
Note how we used mutliple columns in a `data.frame` to calculate a new column and that these operations are performed row-wise, thus for each plant!  
